L.GeoJSON.WFS = L.GeoJSON.extend({
	...
	_onFeatureClick: function(e) {
		var f = e.target.feature,
			shiftKeyWasPressed = e.originalEvent ? e.originalEvent.shiftKey : false, 
			target = e.target,
			paramVal = null,
...
//		setTimeout(function() {
//			var resetStyle = self.resetStyle;
//			self.eachLayer(function(lay) {
//				resetStyle.call(self, lay);
//			});
//			for (var i=0,len=self._selectedFeatures.length; i<len; i++) {
//				_lay = layerFromFeature(self._selectedFeatures[i], self);
//				if (_lay.setStyle) {
//					_lay.setStyle(self.options.selectStyle);
//				}
